...
def expand(array, start, labels, currentLabel, clusterCenters, version):  # TODO
    ...
    while expansionQueue:
        ...
        for neigbour in neigbours:
            ...
            if point == (24, 13):
                a = 4
            if (not visitet[location]) and \
                    (0 < array[location] <= array[point]):
                # (0.7<abs((array[location] - array[point])/array[clusterCenters[currentLabel-1]]) < 1.3 ):
...
def disambiguate2(array, questionPoint, cluster1, cluster2, threshold=0):
    # MERGE
    if (cluster1 == questionPoint) or (cluster2 == questionPoint):
        if array[cluster1] > array[cluster2]:
            return 11
        elif array[cluster1] < array[cluster2]:
            return 22
    ...
    c1Strength = (array[cluster1] / pointStrength) / distanceToC1
    c2Strength = (array[cluster2] / pointStrength) / distanceToC2
    if (abs(c1Strength - c2Strength) <= threshold):
        return 0
    if (c1Strength - c2Strength > 0):
        return 1
    elif (c1Strength - c2Strength < 0):
        return 2

def disambiguate(array, questionPoint, cluster1, cluster2, threshold=0):
    if (cluster1 == questionPoint) or (cluster2 == questionPoint):
        if array[cluster1] > array[cluster2]:
            return 11
        else:
